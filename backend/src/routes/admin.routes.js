  ...
  try {
    ...
    const partnersWithStats = await Promise.all(
      partners.map(async (partner) => {
        const totalLeads = await Lead.count({ where: { user_id: partner.id } });
        const contratos = await Lead.count({
          where: {
            user_id: partner.id,
...
// Buscar uma indicação específica (detalhes)
...
// Fechar contrato (COM ATUALIZAÇÃO DO TOTAL_GANHO)
...
// Marcar comissão como paga (COM ATUALIZAÇÃO DO TOTAL_GANHO)
router.put("/commissions/:id/pay", adminMiddleware, async (req, res) => {
  try {
    ...
    // ATUALIZAR TOTAL GANHO DO PARCEIRO (garantindo que não soma duplicado)
    const partner = await User.findByPk(lead.user_id);
    if (partner) {
      // Verificar se já foi contabilizado antes (evitar duplicidade)
      // A comissão já foi somada no fechamento do contrato
      // Se quiser manter apenas no fechamento, comente a linha abaixo
      // partner.total_commission = (parseFloat(partner.total_commission || 0) + parseFloat(lead.commission_value || 0));
      // await partner.save();

      // Já atualizado no fechamento, não precisa somar novamente
      console.log(
        `Comissão paga para ${partner.name} no valor de R$ ${lead.commission_value}`,
      );
    }
